    ...
    for possible in word_list:
        if len(possible) == len(word):
            for i in range(len(word)):
                if word[i] == "*" or word[i] == possible[i]:
                    if i == len(word)-1:
                        in_wordlist = True
                    else:
                        continue
...
def play_hand(hand, word_list):
    ...
    * The hand finishes when there are no more unused letters.
    ...
    if n == 0:
        ...
        print("\n" + 
        "Game has been ended. Total score: " + str(total_score) + " points" +
        "\n", end = " ")
        print("---------------------------------------")
...
def substitute_hand(hand, letter):
    ...
    hand_delete = dict()
    for letter1 in handc.keys():
        hand_delete[letter1] = ""
    
    alphabet_without_hand = alphabet.translate(str.maketrans(hand_delete))

    if letter in handc.keys():
        x = random.choice(alphabet_without_hand)
        y = handc[letter]
        handc.pop(letter, None)
        handc[x] = y
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while num_hands > 0:
        ...
        if sub == "yes":
            repl_letter = input("Which letter would you like to replace? ")
            hand = substitute_hand(hand, repl_letter)

        total_score1 = play_hand(hand, word_list)
